...
import { MaterialConnectionCRUDAction } from "api/igc";
import { MaterialConnectionSerializer } from "api/types/igcTypes";
...
const _dialogName = "MODAL_IGC_MATERIALCONNECTION";
...
const MaterialConnectionDialogForm = () => {
  ...
  const dialogObject = useAppSelector(
    (state) =>
      state.modal.dialogObjects[_dialogName] as MaterialConnectionSerializer
  );
  const {
    register,
    control,
